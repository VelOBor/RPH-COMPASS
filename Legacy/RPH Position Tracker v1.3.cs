...
// Phase 1.3: Hybrid Cleanup Edition (Orphan purge + Ignore flag)
...
//        "Tracked Parts". Automatically revalidates Custom Data.
//
//  cleanup
//      → Manually triggers orphan cleanup (removes RPH data from
//        blocks no longer in the group).
//
//  (planned) calibrate <name or index>
//      → Calibrates zero position for specified part or all parts.
//
//  (planned) debug
//      → Toggles extended debug info on the PB’s LCD.
//
//  (planned) export / import
//      → (Future feature) Saves or loads RPH settings between grids.
//
//  (planned) set <param>=<value>
//      → Adjusts parameters such as update rate or display options.
//
// Notes:
//   • Arguments are typed into the "Run" box in the Programmable Block terminal.
...
Program()
{
    ...
    // Always clear stale references
    parts.Clear();
    groupBlocks.Clear();
    lcds.Clear();

    BuildPartsAndLCDs();
    CleanupOrphans(); // auto-maintenance on compile

    Echo("RPH Tracker v1.3 initialized");
...
void Main(string arg, UpdateType update)
{
    ...
    if (!string.IsNullOrWhiteSpace(arg))
    {
        ...
        if (arg == "cleanup")
        {
            Echo("RPH: Running orphan cleanup...");
            CleanupOrphans();
            return;
        }
    }
...
void BuildPartsAndLCDs()
{
    ...
    {
        var b = groupBlocks[i];
        if (b == null || b.Closed || b.CubeGrid == null || !b.IsFunctional)
            continue;

        if (b.CustomData.ToLower().Contains("rph_ignore=true"))
            continue; // skip ignored parts
    ...
    for (int i = 0; i < groupBlocks.Count; i++)
    {
        ...
        if (b is IMyMotorStator || b is IMyMotorAdvancedStator || b is IMyPistonBase)
            continue;
...
void CleanupOrphans()
{
    ...
    GridTerminalSystem.GetBlocksOfType<IMyTerminalBlock>(allBlocks, b =>
    ...
    {
        if (b == null || b.Closed || b.CubeGrid == null) continue;
        if (groupBlocks.Contains(b)) continue; // still tracked
    ...
    Echo($"RPH: Cleanup complete. Orphans removed: {cleaned}");
}

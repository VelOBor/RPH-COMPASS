...
[r] - featire is removed from the docket until further notice
...
## [x] Phase 2 — Multi-LCD, Sub-Group Filtering
- [x] Include LCDs **only if they are part of the main terminal group** (`GROUP_NAME`).  
- [x] Parse LCD Custom Data lines:  
  ```
  sub-group to display: Tail Extenders
  sub-group to display: Tail Rotors
  ```
- [x] Each LCD filters parts based on sub-group(s).  
- [x] Efficient text-building routine to handle multiple displays without duplication.  
...
## [x] Phase 6 — Compatibility & Safety (High Priority)
- [x] Review **PARK script** (Steam ID `1933151026`) parsing behavior.  
- [x] Ensure non-interference by using unique prefixes, e.g.:
  ```
  rph_display name:
  rph_sub-group:
  ```
- [x] Ignore any non-RPH tags.  
- [x] Define final RPH Custom Data schema & document it clearly.  

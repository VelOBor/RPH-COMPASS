...
Program()
{
    ...
    Echo("RPH COMPASS v1.7 initialized");
...
void HeadsUpMissingHeaders()
{
    ...
    if (missing > 0)
    {
        Echo("WARNING: " + missing + " parts missing RPH header!");
        for (int i = 0; i < names.Count; i++)
            Echo(" - " + names[i]);
        if (missing > names.Count)
            Echo(" - (+" + (missing - names.Count) + " more)");
        Echo("Run 'rebuild_cd' to repopulate. Will RESET all RPH keys to default.");
    }
}
...
void EchoLcdSummary()
{
    ...
    {
        var t = lcdTargets[i];
        Echo("LCD: mode=" + t.Config.Mode + " groups=" + t.Config.GroupsCount);
    }
}

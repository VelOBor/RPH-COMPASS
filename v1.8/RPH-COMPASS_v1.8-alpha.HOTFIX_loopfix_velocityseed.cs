...
void RenderDebug(LcdTarget tgt, List<TrackedPart> list)
{
        ...
        // RPH [LOCKED] indicators (debug-only)
        try
        {
            if (parts != null)
            {
                for (int __i = 0; __i < parts.Count; __i++)
                {
                    var __pb = parts[__i].Block as IMyTerminalBlock;
                    if (__pb != null && IsReadOnly(__pb))
                        sb.AppendLine(parts[__i].DisplayName + " [LOCKED]");
                }
            }
            if (groupBlocks != null)
            {
                for (int __j = 0; __j < groupBlocks.Count; __j++)
                {
                    var __gb = groupBlocks[__j] as IMyTerminalBlock;
                    var __prov = groupBlocks[__j] as IMyTextSurfaceProvider;
                    if (__gb != null && __prov != null && IsReadOnly(__gb))
                        sb.AppendLine(__gb.CustomName + " [LOCKED]");
                }
            }
        }
        catch {}
...
void BuildPartsAndLCDs(bool writeHeaders)
{
    ...
    for (int i = 0; i < groupBlocks.Count; i++)
    {
        ...
        if (s0 != null)
        {
            ...
            if (writeHeaders) EnsureLcdDisplaySectionSafe(b);
...
void CalibrateAll()
{
    ...
    for (int i = 0; i < parts.Count; i++)
    {
        ...
        try
        {
            if (!IsReadOnly(b)) WriteRphSection(b, meta);
...
void CleanupOrphans(bool silent)
{
    ...
    for (int i = 0; i < all.Count; i++)
    {
        ...
        if (!IsReadOnly(b as IMyTerminalBlock)) { b.CustomData = (data.Substring(0, start) + data.Substring(next)).Trim();
        removed++; }
    }
...
// ---------------------------------------------------------------------------
// RPH READ-ONLY + SAFE WRITE WRAPPERS (non-invasive)
// ---------------------------------------------------------------------------
bool IsReadOnly(IMyTerminalBlock b)
{
    if (b == null) return false;
    string cd = b.CustomData;
    if (string.IsNullOrWhiteSpace(cd)) return false;
    var lines = cd.Split('\n');
    for (int i = 0; i < lines.Length; i++)
    {
...
// Safe write wrappers: no-op if block is marked read-only
void EnsureLcdDisplaySectionSafe(IMyTerminalBlock b)
{
    if (IsReadOnly(b)) return;
    EnsureLcdDisplaySection(b);
}


void EnsureRphSectionSafe(IMyTerminalBlock b, RphMeta m)
{
    if (IsReadOnly(b)) return;
    EnsureRphSection(b, m);
}

void WriteRphSectionSafe(IMyTerminalBlock b, RphMeta m)
{
    if (IsReadOnly(b)) return;
    WriteRphSection(b, m);
}

...
// HOTFIX: NUKE clear-RPH arming state
string nukeCode = null;
string nukeScope = null; // ALL | MECH | DISPLAY
long nukeExpiresTicks = 0;
System.Random nukeRand = new System.Random();
...
void Main(string arg, UpdateType update)
{
    ...
    // HOTFIX: NUKE command handler (arm/commit)
    string __cmd = string.IsNullOrWhiteSpace(arg) ? "" : arg.Trim();
    string __cmdU = __cmd.ToUpperInvariant();
    if (HandleNukeCommands(__cmdU)) return;
...
// ---------------------------------------------------------------------------
// HOTFIX: Clear-RPH NUKE (arm/commit)
// ---------------------------------------------------------------------------
bool HandleNukeCommands(string cmdU)
{
    if (string.IsNullOrEmpty(cmdU)) return false;

    // ARM
    if (cmdU == "ARMCLR-NUKE-ALL" || cmdU == "ARMCLR-NUKE-MECH" || cmdU == "ARMCLR-NUKE-DISPLAY")
    {
        nukeScope = cmdU.EndsWith("-ALL") ? "ALL" : (cmdU.EndsWith("-MECH") ? "MECH" : "DISPLAY");
        nukeCode = GenNukeCode(4);
        nukeExpiresTicks = DateTime.UtcNow.AddSeconds(30).Ticks;
        SafeEcho("NUKE armed: scope=" + nukeScope + "  code=" + nukeCode + "  expires in 30s");
        return true;
    }

    // COMMIT
    if (cmdU.StartsWith("DOCLR-"))
    {
        var code = cmdU.Substring(6).Trim();
        if (string.IsNullOrEmpty(nukeCode))
        {
            SafeEcho("NUKE reject: not armed");
            return true;
        }
        if (DateTime.UtcNow.Ticks > nukeExpiresTicks)
        {
            SafeEcho("NUKE reject: expired");
            nukeCode = null; nukeScope = null; nukeExpiresTicks = 0;
            return true;
        }
        if (!string.Equals(code, nukeCode, StringComparison.OrdinalIgnoreCase))
        {
            SafeEcho("NUKE reject: wrong code");
            return true;
        }

        int cleared = 0;
        int targets = 0;
        if (nukeScope == "ALL" || nukeScope == "MECH")
        {
            for (int i = 0; i < parts.Count; i++)
            {
                var b = parts[i].Block;
                if (b == null) continue;
                targets++;
                cleared += ClearRphKeys(b);
            }
        }
        if (nukeScope == "ALL" || nukeScope == "DISPLAY")
        {
            for (int i = 0; i < lcdTargets.Count; i++)
            {
                var lb = lcdTargets[i].Block;
                if (lb == null) continue;
                targets++;
                cleared += ClearRphKeys(lb);
            }
        }

        SafeEcho("NUKE done: scope=" + nukeScope + "  targets=" + targets + "  keys_cleared=" + cleared);
...
string GenNukeCode(int len)
{
    const string alpha = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    var sb = new StringBuilder(len);
    for (int i = 0; i < len; i++)
        sb.Append(alpha[nukeRand.Next(alpha.Length)]);
    return sb.ToString();
}

// Remove all lines beginning with 'rph_' (case-insensitive) from CustomData
int ClearRphKeys(IMyTerminalBlock b)
{
    var cd = b.CustomData;
    if (string.IsNullOrEmpty(cd)) return 0;
    var lines = cd.Split('\n');
    int removed = 0;
    ...
    {
        var raw = lines[i];
        var trim = raw.TrimStart();
        if (trim.Length >= 4 && (trim.StartsWith("rph_", StringComparison.OrdinalIgnoreCase)))
        {
            removed++;
            continue;
        }
    ...
    if (removed > 0) b.CustomData = sb.ToString();
    return removed;
}

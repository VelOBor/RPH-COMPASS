...
Program()
{
    ...
    Echo("RPH Tracker v1.6e initialized");
...
void RenderFull(LcdTarget tgt, List<TrackedPart> list)
{
    ...
    {
        sb.AppendLine(list[p].GetDisplayLine());
        if (list[p].IsCalibrated) calibratedCount++;
    }

    sb.AppendLine();
    sb.AppendLine("Parts shown: " + list.Count);
    sb.AppendLine("Calibrated: " + calibratedCount + " / " + list.Count);
...
void DoRefresh(bool writeHeaders)
{
    ...
    cd.AppendLine("rph_header = RPH Tracker");
...
// ---------------------------------------------------------------------------
//
// Version: 1.6e (Display modes: full | compact | debug)

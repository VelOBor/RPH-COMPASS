...
// Phase 3c: Command split (refresh vs rebuild_cd) and ASCII-only outputs
...
//
//  rebuild_cd
//      -> Rebuild RPH headers for parts and LCDs (WRITES Custom Data).
...
Program()
{
    ...
    // Was: BuildPartsAndLCDs(false);
    DoRefresh(false); // read-only: builds lists AND writes PB [RPH:Info]/[RPH:Commands]
    HeadsUpMissingHeaders();

    Echo("RPH Tracker v1.6c initialized");
...
void DoRefresh(bool writeHeaders)
{
    ...
    cd.AppendLine("rebuild_cd    -> Recreate RPH headers (writes Custom Data)");
...
// Full-grid orphan cleanup: remove [RPH:Tracking] from mechanical parts
// that are no longer in the "Tracked Parts" group.
...
void HeadsUpMissingHeaders()
{
    ...
    if (missing > 0)
        Echo("Heads-up: " + missing + " parts are missing [RPH:Tracking]. Run 'rebuild_cd' to initialize headers.");
}
...
// ---------------------------------------------------------------------------
//
// Version: 1.6c (Command split + ASCII-only)
// - refresh: read-only rescan
// - rebuild_cd: write missing headers
// - calibrate_all: explicit flag rph_is_calibrated
// - ASCII-only strings in Echo and Custom Data
// - PARK-safe and backward compatible
// ---------------------------------------------------------------------------
